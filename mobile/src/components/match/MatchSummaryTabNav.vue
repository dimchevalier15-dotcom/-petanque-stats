<template>
  <nav class="summary-segments" role="tablist" :aria-label="t('summary.tabs.label')">
    <div class="summary-segments-track">
      ...
      >
        ...
        <span>{{ t('summary.tabs.overview') }}</span>
      </button>
      <button
        type="button"
        role="tab"
      ...
      >
        ...
        <span>{{ t('summary.tabs.tactics') }}</span>
      </button>
    </div>
  </nav>
</template>
...
export type MatchSummaryTab = 'overview' | 'tactics'

defineProps<{
  modelValue: MatchSummaryTab
}>()

defineEmits<{
  'update:modelValue': [value: MatchSummaryTab]
}>()
